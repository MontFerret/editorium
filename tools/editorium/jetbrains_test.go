...
func TestJetBrainsGradleUsesIntegrationLocalWrapperAndEnvironment(t *testing.T) {
	...
	writeTestFile(t, wrapper, []byte("#!/bin/sh\nprintf '%s|%s\\n' \"$FERRETD_TEST_PATH\" \"$*\" >> \"$JETBRAINS_TEST_LOG\"\n"), 0o755)
	...
	for _, arguments := range [][]string{
		...
		if err := runJetBrainsGradle(context.Background(), root, arguments...); err != nil {
			t.Fatal(err)
		}
	}
	if err := runJetBrainsGradleEnvironment(
		context.Background(),
		root,
		[]string{"FERRETD_TEST_PATH=/tmp/pinned-ferretd"},
	...
	for _, expected := range []string{
		...
		"|compileKotlin compileTestKotlin verifyPluginProjectConfiguration verifyPluginStructure verifyPlugin",
		"|buildPlugin verifyPluginProjectConfiguration verifyPluginStructure verifyPlugin",
		"/tmp/pinned-ferretd|ferretdIntegrationTest",
	} {
...
func TestPrepareJetBrainsTestFerretdAcquiresOnlyTheCurrentHost(t *testing.T) {
	root := pinnedTestRoot(t)
	archives, checksums := jetbrainsReleaseFixtures(t)
	requests := 0
	prepared, err := prepareJetBrainsTestFerretd(context.Background(), root, doerFunc(func(request *http.Request) (*http.Response, error) {
		requests++
	...
	preparedTarget := filepath.Base(filepath.Dir(filepath.Dir(prepared.BinaryPath)))
	if preparedTarget != target.ID {
		t.Fatalf("prepared target = %s, want %s", preparedTarget, target.ID)
	}

	assertTestContents(t, prepared.BinaryPath, "binary "+target.ID)
}
